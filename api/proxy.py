...
<RuleList>
    ...
    for key in ok_list:
        ...
        ppx_name=str(time.time_ns())+'.ppx'
    with open(file_path+'/result/'+ppx_name, 'a') as s:
        s.write(temp_1+temp_2+temp_3)
    print('验证完毕，结果位于根目录下的'+ppx_name)


def check_proxy(all_proxy,file_path):
    ok_list={}
    socks=['socks5']
    proxy_file=open(file_path+"/result/"+str(time.time_ns())+".txt",'a',encoding='utf-8')
    for sock in socks:
