...
def icp_domains(domain,excel,xlsx_save_name):
    ...
    #写入数据的准备工作
    data=["目标","域名主办方","域名","备案许可证号","网站备案号","域名类型","网站前置审批项","是否限制接入","审核通过日期"]
    all_sheet=excel.sheetnames

    if "icp备案" in all_sheet:         #查看是否存在icp备案这个sheet
        w_excel=excel["icp备案"]
    else:
        w_excel=excel.create_sheet("icp备案",0)            #如果没有则新建
...
def icp_check(domains,excel,xlsx_save_name):
    all_icp_domains=[]
    for domain in domains:
        domain_list=icp_domains(domain,excel,xlsx_save_name)
        for i in domain_list:
            all_icp_domains.append(i)
    return all_icp_domains